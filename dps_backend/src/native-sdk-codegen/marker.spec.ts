...
describe('replaceMarkedRegion', () => {
  ...
  it('works with a hash comment marker', () => {
    ...
    expect(replaceMarkedRegion(podfile, 'PODS', "  pod 'X'")).toContain(
      "  pod 'X'",
    );
  });
  ...
  it('throws when the end marker is missing', () => {
    ...
    expect(() => replaceMarkedRegion(broken, 'IMPORTS', 'x')).toThrow(
      /end marker/i,
    );
  });
  ...
  it('throws when the markers appear more than once', () => {
    ...
    expect(() => replaceMarkedRegion(dup, 'IMPORTS', 'x')).toThrow(
      /more than once/i,
    );
  });
});

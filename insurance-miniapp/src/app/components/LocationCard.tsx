...
declare global {
  interface Window {
    ...
    dspCallback?: (callbackId: string, data: any) => void;
  }
}
...
export default function LocationCard() {
  ...
  useEffect(() => {
    ...
    window.dspCallback = (callbackId: string, data: any) => {
  ...
  const requestLocation = () => {
    ...
    if (window.DSPNativeBridge) {
      ...
      try {
        window.DSPNativeBridge.postMessage(JSON.stringify(payload));
      } catch (err) {
      ...
      setError('Native Bridge not found. Please open this app inside the DSP Super App.');
      setLoading(false);
    }
  };

...
export default function BiometricsCard() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const originalCallback = window.dspCallback;
      window.dspCallback = (callbackId: string, response: any) => {
        if (callbackId === 'auth-request') {
  ...
  const handleAuthenticate = () => {
    ...
    if (typeof window !== 'undefined' && window.DSPNativeBridge) {
      window.DSPNativeBridge.postMessage(JSON.stringify({
        action: 'authenticate',
        callbackId: 'auth-request'
      }));
    } else {
      setTimeout(() => {
        setError("Not running inside DSP Super App");
        setIsAuthenticating(false);
      }, 500);
    }
  };
